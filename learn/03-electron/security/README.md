...
DJ.ai implements all six security layers. CSP headers are injected via `session.webRequest.onHeadersReceived` in `main.cjs`. All windows have `nodeIntegration: false` and `contextIsolation: true`. The `setWindowOpenHandler` blocks unauthorized popups. Safe storage encrypts API keys with OS-level encryption. Rate limiting on the decrypt endpoint prevents brute-force attacks. These layers work together so that even if a third-party page (OAuth providers) is compromised, it cannot escape the sandbox.
